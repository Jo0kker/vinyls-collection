<div class="bg-white" :class="{ 'ring-1 ring-blue-500': state.selectedThreads.includes({{ $thread->id }}) }">
    <div class="flex flex-col md:items-start md:flex-row md:justify-between md:gap-4 p-6">
        <div class="md:w-3/6 text-center md:text-left">
            <span class="lead">
                <a href="{{ Forum::route('thread.show', $thread) }}" @if (isset($category))style="color: {{ $category->color_light_mode }};"@endif class="text-link hover:text-accent transition text-lg">{{ $thread->title }}</a>
            </span>
            <br>
            {{ $thread->authorName }}
            ...
            @if (!isset($category))
                <br>
                <a href="{{ Forum::route('category.show', $thread->category) }}" class="category-title text-link hover:text-accent transition">{{ $thread->category->title }}</a>
            @endif
        </div>
        ...
        @if ($thread->lastPost)
            <div class="md:w-1/6 flex justify-center md:flex-col md:items-end">
                <a href="{{ Forum::route('thread.show', $thread->lastPost) }}" class="text-link hover:text-accent transition">{{ trans('forum::posts.view') }} &raquo;</a>
                <div>
