<nav aria-label="breadcrumb" class="mb-4">
    <ol class="flex flex-wrap [&_li]:after:content-['/'] [&_li]:after:px-2 [&_li]:after:text-gray-500 [&_li:last-child]:after:content-['']">
        <li class=""><a href="{{ url(config('forum.frontend.router.prefix')) }}" class="text-link hover:text-accent transition">{{ trans('forum::general.index') }}</a></li>
        @if (isset($category) && $category)
            @include ('forum::partials.breadcrumb-categories', ['category' => $category])
        @endif
        @if (isset($thread) && $thread)
            <li class=""><a href="{{ Forum::route('thread.show', $thread) }}" class="text-link hover:text-accent transition">{{ $thread->title }}</a></li>
        @endif
