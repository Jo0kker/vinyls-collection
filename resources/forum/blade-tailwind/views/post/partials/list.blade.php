<div @if (!$post->trashed())id="post-{{ $post->sequence }}"@endif
    class="border mb-2 rounded-md {{ $post->trashed() || $thread->trashed() ? 'opacity-50' : '' }}"
    :class="{ 'border-blue-500': state.selectedPosts.includes({{ $post->id }}) }">
    <div class="bg-gray-700 border-b px-6 py-4 flex justify-between flex-row-reverse rounded-t-md">
