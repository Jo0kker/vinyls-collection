    ...
    <nav class="v-navbar bg-white shadow py-4">
        <div class="container mx-auto px-4 md:flex md:items-center md:gap-4">
            <div class="flex justify-between items-center">
                <a class="text-lg font-semibold" href="{{ url(config('forum.frontend.router.prefix')) }}">Vinyls Collection</a>
            ...
            <div class="grow justify-between navbar-collapse" :class="{ 'flex flex-col': !isCollapsed, 'hidden md:flex': isCollapsed }">
                ...
                <ul class="navbar-nav flex gap-4 flex-col md:flex-row">
                    ...
                    @else
                        <li class="nav-item">
                            <a class="nav-link" href="{{ url('/login') }}">{{ trans('forum::general.login') }}</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link" href="{{ url('/register') }}">{{ trans('forum::general.register') }}</a>
                        </li>
                    @endif
                </ul>
            </div>
